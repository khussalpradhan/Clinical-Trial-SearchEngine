...
def render_results(results):
    ...
    st.markdown("---")
    st.markdown(f"<p style='color:#aaa; font-size:13px;'>{len(results)} trial(s) found</p>", unsafe_allow_html=True)
    ...
    for idx, trial in enumerate(results):
        ...
        with st.container():
            # Card header rendered via HTML (no interactive elements inside)
            st.markdown(
                f"""
                <div class="card-box">
            ...
            # Button OUTSIDE the HTML block so Streamlit can render it
            if st.button("View Details →", key=f"view_{idx}"):
                if nct_id:
                    ...
                    if details:
                        ...
                        st.error("Could not fetch trial details.")
                else:
                    # Fallback: use whatever data we already have from the hit
